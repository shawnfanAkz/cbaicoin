CI AI COIN Core version 0.18.0 is now available from:

  <https://CI AI COINcore.org/bin/CI AI COIN-core-0.18.0/>
...
`contrib/init/CI AI COINd.service`, it has been changed to use
`/var/lib/CI AI COINd` as the data directory instead of
`~CI AI COIN/.CI AI COIN`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/CI AI COINd` will
exist has enough space (check using `df -h /var/lib/CI AI COINd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/CI AI COIN/CI AI COIN/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://CI AI COINcore.org/en/contact/) the CI AI COIN Core
...
The systemd init file (`contrib/init/CI AI COINd.service`) has been changed
to use `/var/lib/CI AI COINd` as the data directory instead of
`~CI AI COIN/.CI AI COIN`. This change makes CI AI COIN Core more consistent with
...
`datadir` option in `/etc/CI AI COIN/CI AI COIN.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/CI AI COIN/CI AI COIN.conf`.


Documentation
-------------

- A new short [document](https://github.com/CI AI COIN/CI AI COIN/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/CI AI COIN/CI AI COIN/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/CI AI COIN/CI AI COIN/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/CI AI COIN/CI AI COIN/blob/master/doc/CI AI COIN-conf.md)
  about the `CI AI COIN.conf` file describes how to use it to configure
  CI AI COIN Core.

- A new document introduces CI AI COIN Core's BIP174 [Partially-Signed
  CI AI COIN Transactions
  (PSBT)](https://github.com/CI AI COIN/CI AI COIN/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/CI AI COIN/CI AI COIN/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent CI AI COIN-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  CI AI COIN Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/CI AI COIN/CI AI COIN/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/CI AI COIN/CI AI COIN/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when CI AI COIN Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of CI AI COIN Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/CI AI COIN/CI AI COIN/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/CI AI COIN/CI AI COIN/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart CI AI COINd with the `-deprecatedrpc=generate`
...
- A new `CI AI COIN-wallet` tool is now distributed alongside CI AI COIN
...
This section describes planned changes to CI AI COIN Core that may affect
other CI AI COIN software and services.

- Since version 0.16.0, CI AI COIN Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with CI AI COIN Core 0.20 (expected about
  a year after 0.18), CI AI COIN Core will default to native segwit
  ...
  to bech32 addresses, and if the CI AI COIN Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in CI AI COIN Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any CI AI COIN Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, CI AI COIN Core
...
- A sub-project of CI AI COIN Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with CI AI COIN Core.  See their [project
  page](https://github.com/CI AI COIN-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to CI AI COIN Core's own implementation, although entropy
  gathered by CI AI COIN Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves CI AI COIN Core a
...
- On macOS, CI AI COIN Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `CI AI COIN_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libCI AI COIN_server.a` (jnewbery)
...
- #13926 [Tools] CI AI COIN-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like CI AI COINd (MarcoFalke)
...
- #14521 qt, docs: Fix `CI AI COIN-qt -version` output formatting (hebasto)
...
- #11625 Add CI AI COINApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libCI AI COIN_server` (laanwj)
...
- #15154 Configure: CI AI COIN-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/CI AI COIN-wallet (Sjors)
...
- #11526 Visual Studio build configuration for CI AI COIN Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libCI AI COIN_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in CI AI COIN-util-test.py (isghe)
...
- #12775 Integration of property based testing into CI AI COIN Core (Christewart)
...
- #12246 Bugfix: Only run CI AI COIN-tx tests when CI AI COIN-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_CI AI COIN` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_CI AI COIN` (MarcoFalke)
...
- #14186 CI AI COIN-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_CI AI COIN.conf` message on startup if conf file exists (leishman)
...
- #12255 Update CI AI COIN.service to conform to init.md (dongcarl)
...
- #14497 Add doc/CI AI COIN-conf.md (hebasto)
...
- #15354 Add missing `CI AI COIN-wallet` tool manpages (MarcoFalke)
...
- #15765 correct CI AI COINconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support CI AI COIN folder on OSX (JimmyMow)
