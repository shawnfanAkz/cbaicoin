CB AI COIN Core version 0.18.0 is now available from:

  <https://CB AI COINcore.org/bin/CB AI COIN-core-0.18.0/>
...
`contrib/init/CB AI COINd.service`, it has been changed to use
`/var/lib/CB AI COINd` as the data directory instead of
`~CB AI COIN/.CB AI COIN`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/CB AI COINd` will
exist has enough space (check using `df -h /var/lib/CB AI COINd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/CB AI COIN/CB AI COIN/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://CB AI COINcore.org/en/contact/) the CB AI COIN Core
...
The systemd init file (`contrib/init/CB AI COINd.service`) has been changed
to use `/var/lib/CB AI COINd` as the data directory instead of
`~CB AI COIN/.CB AI COIN`. This change makes CB AI COIN Core more consistent with
...
`datadir` option in `/etc/CB AI COIN/CB AI COIN.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/CB AI COIN/CB AI COIN.conf`.


Documentation
-------------

- A new short [document](https://github.com/CB AI COIN/CB AI COIN/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/CB AI COIN/CB AI COIN/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/CB AI COIN/CB AI COIN/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/CB AI COIN/CB AI COIN/blob/master/doc/CB AI COIN-conf.md)
  about the `CB AI COIN.conf` file describes how to use it to configure
  CB AI COIN Core.

- A new document introduces CB AI COIN Core's BIP174 [Partially-Signed
  CB AI COIN Transactions
  (PSBT)](https://github.com/CB AI COIN/CB AI COIN/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/CB AI COIN/CB AI COIN/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent CB AI COIN-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  CB AI COIN Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/CB AI COIN/CB AI COIN/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/CB AI COIN/CB AI COIN/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when CB AI COIN Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of CB AI COIN Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/CB AI COIN/CB AI COIN/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/CB AI COIN/CB AI COIN/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart CB AI COINd with the `-deprecatedrpc=generate`
...
- A new `CB AI COIN-wallet` tool is now distributed alongside CB AI COIN
...
This section describes planned changes to CB AI COIN Core that may affect
other CB AI COIN software and services.

- Since version 0.16.0, CB AI COIN Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with CB AI COIN Core 0.20 (expected about
  a year after 0.18), CB AI COIN Core will default to native segwit
  ...
  to bech32 addresses, and if the CB AI COIN Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in CB AI COIN Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any CB AI COIN Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, CB AI COIN Core
...
- A sub-project of CB AI COIN Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with CB AI COIN Core.  See their [project
  page](https://github.com/CB AI COIN-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to CB AI COIN Core's own implementation, although entropy
  gathered by CB AI COIN Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves CB AI COIN Core a
...
- On macOS, CB AI COIN Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `CB AI COIN_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libCB AI COIN_server.a` (jnewbery)
...
- #13926 [Tools] CB AI COIN-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like CB AI COINd (MarcoFalke)
...
- #14521 qt, docs: Fix `CB AI COIN-qt -version` output formatting (hebasto)
...
- #11625 Add CB AI COINApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libCB AI COIN_server` (laanwj)
...
- #15154 Configure: CB AI COIN-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/CB AI COIN-wallet (Sjors)
...
- #11526 Visual Studio build configuration for CB AI COIN Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libCB AI COIN_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in CB AI COIN-util-test.py (isghe)
...
- #12775 Integration of property based testing into CB AI COIN Core (Christewart)
...
- #12246 Bugfix: Only run CB AI COIN-tx tests when CB AI COIN-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_CB AI COIN` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_CB AI COIN` (MarcoFalke)
...
- #14186 CB AI COIN-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_CB AI COIN.conf` message on startup if conf file exists (leishman)
...
- #12255 Update CB AI COIN.service to conform to init.md (dongcarl)
...
- #14497 Add doc/CB AI COIN-conf.md (hebasto)
...
- #15354 Add missing `CB AI COIN-wallet` tool manpages (MarcoFalke)
...
- #15765 correct CB AI COINconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support CB AI COIN folder on OSX (JimmyMow)
