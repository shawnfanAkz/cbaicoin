CI AI COIN Core version 0.11.0 is now available from:

  <https://CI AI COIN.org/bin/CI AI COIN-core-0.11.0/>
...
For example, add the following to `CI AI COIN.conf`:
...
related to the blockchain in the CI AI COIN system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows CI AI COIN Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that CI AI COIN Core will 
...
For an example script see [CI AI COIN-submittx](https://github.com/laanwj/CI AI COIN-submittx).
...
- #5334 `cf87536` libCI AI COINconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new CI AI COIN-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only CI AI COINAmountField
- #5753 `0f44672` Add CI AI COIN logo to about screen
...
- #5809 `46bfbe7` Add CI AI COIN-cli man page
...
- #5849 `41259ca` contrib/init/CI AI COINd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename CI AI COIN-Qt.app to CI AI COIN-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for CI AI COIN-tx)
