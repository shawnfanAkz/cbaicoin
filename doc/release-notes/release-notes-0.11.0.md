CB AI COIN Core version 0.11.0 is now available from:

  <https://CB AI COIN.org/bin/CB AI COIN-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer CICX/kB fee to be
...
For example, add the following to `CB AI COIN.conf`:
...
related to the blockchain in the CB AI COIN system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows CB AI COIN Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that CB AI COIN Core will 
...
For an example script see [CB AI COIN-submittx](https://github.com/laanwj/CB AI COIN-submittx).
...
- #5334 `cf87536` libCB AI COINconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new CB AI COIN-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only CB AI COINAmountField
- #5753 `0f44672` Add CB AI COIN logo to about screen
...
- #5809 `46bfbe7` Add CB AI COIN-cli man page
...
- #5849 `41259ca` contrib/init/CB AI COINd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename CB AI COIN-Qt.app to CB AI COIN-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for CB AI COIN-tx)
