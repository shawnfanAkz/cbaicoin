...
- [Development guidelines](#development-guidelines)
    - [General CB AI COIN Core](#general-CB AI COIN-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
CB AI COIN Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play CB AI COINs" on the test network, if you
...
CB AI COIN Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/CB AI COIN/CB AI COIN/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_CB AI COIN
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_CB AI COIN --log_level=test_suite
$ valgrind -v --leak-check=full src/CB AI COINd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_CB AI COIN.coverage/index.html`.
```
...
To profile a running CB AI COINd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep CB AI COINd` -- sleep 60
```
...
CB AI COIN Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/CB AI COIN/CB AI COIN/issues/12691)
...
- ThreadRPCServer : Remote procedure call handler, listens on port 28832 for connections and services them.
...
However, in open source software such as CB AI COIN Core, where everyone uses
...
pay attention to for reviewers of CB AI COIN Core code.

General CB AI COIN Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: CB AI COIN Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of CB AI COIN Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against CB AI COIN Core so that they can be integrated
...
- src/libsecp256k1
  - Upstream at https://github.com/CB AI COIN-core/secp256k1/ ; actively maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/CB AI COIN-core/ctaes ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/CB AI COIN-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors, it will cause problems with CB AI COIN's `select()` loop, because
...
compatibility between nodes. This happened in CB AI COIN 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to CB AI COIN's
...
Commit [`bb81e173`](https://github.com/CB AI COIN/CB AI COIN/commit/bb81e173) is an example of a scripted-diff.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `CB AI COIN-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `CB AI COIN-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of CB AI COIN Core, the wallet was always
